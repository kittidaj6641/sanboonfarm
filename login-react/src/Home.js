...
import { 
    BarChart, Info, Phone, LogOut, Search,
    Fish, AlertTriangle, Clock, Shrimp, Activity 
} from 'lucide-react'; 
...
axios.get(`${config.API_BASE_URL}/member/water-quality`, {
...
(err.response?.data?.error || err.message));
    ...
    const handleLogout = async () => {
        ...
        try {
            const response = await
axios.post(`${config.API_BASE_URL}/member/logout`, {}, {
                headers: { Authorization: `Bearer ${token}` }
            });
    ...
    const chartData = latestData
        ? [
            { name: 'ความเค็ม (ppt)', value:
Number(latestData.salinity) || 0 },
            { name: 'pH', value: Number(latestData.ph) || 0 },
            { name: 'ออกซิเจน (mg/L)', value:
Number(latestData.dissolved_oxygen) || 0 },
            { name: 'ไนโตรเจน (mg/L)', value:
Number(latestData.nitrogen) || 0 },
            { name: 'ไฮโดรเจนซัลไฟด์ (mg/L)', value:
Number(latestData.hydrogen_sulfide) || 0 },
            { name: 'BOD (mg/L)', value: Number(latestData.bod) || 0 },
            { name: 'อุณหภูมิ (°C)', value:
Number(latestData.temperature) || 0 },
        ]
        : [
    ...
    const checkAlerts = () => {
        ...
        if (latestData.ph < 7.0 || latestData.ph > 8.5) {
            alerts.push(`pH (${latestData.ph}) อยู่นอกเกณฑ์
(ควรอยู่ระหว่าง 7.0-8.5)`);
        }

        if (latestData.dissolved_oxygen < 5) {
            alerts.push(`ออกซิเจนละลายน้ำ (${latestData.dissolved_oxygen}
mg/L) ต่ำเกินไป (ควร ≥ 5 mg/L)`);
        }
        ...
        if (latestData.hydrogen_sulfide > 0.1) {
            alerts.push(`ไฮโดรเจนซัลไฟด์ (${latestData.hydrogen_sulfide}
mg/L) สูงเกินไป (ควร ≤ 0.1 mg/L)`);
        }
    ...
    const handleAlertClick = () => {
        ...
        openModal('⚠️การแจ้งเตือนคุณภาพน้ำ', alertContent);
    };
    ...
    const fetchLoginLogs = async () => {
        ...
        try {
            const response = await
axios.get(`${config.API_BASE_URL}/member/login-logs`, {
                headers: { Authorization: `Bearer ${token}` }
            });
...
index) => `
                                    <tr>
                                        ...
                                        <td class="${log.status === 'online' ? 'status-online' :
'status-offline'}">${log.status}</td>
                                    </tr>
                                `
                                )
                                .join('')}
            ...
            if (err.response) {
                errorMessage += `: ${err.response.status} - ${err.response.data?.error
|| err.message}`;
            } else if (err.request) {
...
/> ข้อมูลคุณภาพน้ำ
                    ...
                    <a
                        ...
                        onClick={(e) => {
                            ...
                            openModal(
                                'ℹ️ เกี่ยวกับเรา',
                                'ฟาร์มกุ้งก้ามกรามคุณภาพสูง
ก่อตั้งปี 2545 ด้วยประสบการณ์กว่า 20
ปี ในการผลิตและพัฒนากุ้งก้ามกรามที่มีคุณภาพสูง ทั้งในด้านการเพาะพันธุ์ การดูแล
และการเก็บเกี่ยว
เรามุ่งเน้นการใช้เทคโนโลยีและวิธีการที่ทันสมัยเพื่อให้ได้ผลผลิตที่ดีที่สุด
สำหรับลูกค้าของเรา พร้อมทั้งสร้างความยั่งยืนในธุรกิจและการดูแลสิ่งแวดล้อม
ด้วยการใช้ระบบจัดการคุณภาพน้ำที่ได้มาตรฐาน
เพื่อให้มั่นใจในความปลอดภัยและคุณภาพของกุ้งที่เราให้บริการ'
                            );
                        }}
                    >
                        <Info size={18}
