...
import {
  ...
  Clock,
  Shrimp,
  Activity, // 🚀 1. เพิ่มไอคอน 'Activity'
} from 'lucide-react';
...
const Home = () => {
  ...
  const chartData = latestData
    ? [
        ...
        {
          name: 'ออกซิเจน (mg/L)',
          value: Number(latestData.dissolved_oxygen) || 0,
        },
        { name: 'ไนโตรเจน (mg/L)', value: Number(latestData.nitrogen) || 0 },
        {
          name: 'ไฮโดรเจนซัลไฟด์ (mg/L)',
          value: Number(latestData.hydrogen_sulfide) || 0,
        },
        { name: 'BOD (mg/L)', value: Number(latestData.bod) || 0 },
        {
          name: 'อุณหภูมิ (°C)',
          value: Number(latestData.temperature) || 0,
        },
      ]
    : [
  ...
  const checkAlerts = () => {
    ...
    if (latestData.salinity < 0 || latestData.salinity > 15) {
      alerts.push(
        `ความเค็ม (${latestData.salinity} ppt) อยู่นอกเกณฑ์ (ควรอยู่ระหว่าง 0-15 ppt)`
      );
    }

    if (latestData.ph < 7.0 || latestData.ph > 8.5) {
      alerts.push(
        `pH (${latestData.ph}) อยู่นอกเกณฑ์ (ควรอยู่ระหว่าง 7.0-8.5)`
      );
    }

    if (latestData.dissolved_oxygen < 5) {
      alerts.push(
        `ออกซิเจนละลายน้ำ (${latestData.dissolved_oxygen} mg/L) ต่ำเกินไป (ควร ≥ 5 mg/L)`
      );
    }

    if (latestData.nitrogen > 1) {
      alerts.push(
        `ไนโตรเจน (${latestData.nitrogen} mg/L) สูงเกินไป (ควร ≤ 1 mg/L)`
      );
    }

    if (latestData.hydrogen_sulfide > 0.1) {
      alerts.push(
        `ไฮโดรเจนซัลไฟด์ (${latestData.hydrogen_sulfide} mg/L) สูงเกินไป (ควร ≤ 0.1 mg/L)`
      );
    }
    ...
    if (latestData.temperature < 26 || latestData.temperature > 32) {
      alerts.push(
        `อุณหภูมิ (${latestData.temperature}°C) อยู่นอกเกณฑ์ (ควรอยู่ระหว่าง 26-32°C)`
      );
    }
  ...
  const fetchLoginLogs = async () => {
    ...
    try {
      // (แก้ไขการตัดบรรทัดที่ผิด)
      const response = await axios.get(
        `${config.API_BASE_URL}/member/login-logs`,
        {
          headers: { Authorization: `Bearer ${token}` },
        }
      );
    ...
    >
      <header className="header">
        <nav className="nav">
          ...
          <button className="history-btn" onClick={fetchLoginLogs}>
      ...
      <div className="main-content">
        ...
        <div className="content-right">
          <h1>
            <Fish size={32} /> ยินดีต้อนรับสู่ฟาร์มกุ้งก้ามกราม
          </h1>
          ...
          {/* 🚀 3. เพิ่มปุ่ม Realtime และจัดกลุ่มปุ่มใหม่ */}
